...
path<-"C:/Users/sgresendez/OneDrive - University of Texas at El Paso/Moon_Cyanbacteria"
...
df1<-map(df[1:4], ~ select(., c("date", "sampleID","onedrivepath", "jpg_filename", "onedriveimagepath", "lid_nolid", "original_retake")))
...
# Select photos to use for phenoanalyser
...
# format filepath info to copy files from original date folders into sample-name specific folders
filenotes.copy <- filenotes.filter %>%
  mutate(cd="cd",
         current_folder=onedrivepath,
         copy="copy",
         file=jpg_filename,
         # PICK UP HERE: NEED TO ADD QUOTES AROUND EACH PIECE OF THE ONEDRIVE PATH
         newdestfolder = paste(paste("C:/Users/sgresendez/OneDrive", "-", "University", "of", "Texas", "at", "El", "Paso/Moon_Cyanbacteria",sep=" "),"Images_SampleID",sampleID,sep="/"),
         pipe="|") %>%
  select(copy,onedriveimagepath,newdestfolder,pipe)
 
# add quotes around all spaces in the newdestfolder path

filenotes.copy <- filenotes.copy %>%
  mutate(newdestfolder = )
...
#save Phenoanalyser list 
write.table(filenotes.pheno,row.names=FALSE, col.names = FALSE,
            file="PhenoanalyzerLists_20221018_20230109.txt",sep=',')


# save copy instructions file
setwd("C:/Users/sgresendez/OneDrive - University of Texas at El Paso/Moon_Cyanbacteria/Images_SampleID")

write.table(filenotes.copy,row.names=FALSE, col.names = FALSE,
            file="CopyImagesToSampleID.txt",sep=' ',quote=FALSE)
